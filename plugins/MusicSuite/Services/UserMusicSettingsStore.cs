...
public sealed class UserMusicSettingsStore : IUserMusicSettingsStore
{
    ...
    public UserMusicSettingsStore(ILogger<UserMusicSettingsStore> logger)
        : this(
            () => Plugin.Instance?.DataFolderPath
                ?? Path.Combine(AppContext.BaseDirectory, "MusicSuite"),
            logger)
    {
    }

    /// <summary>
    /// Initializes a new instance of the <see cref="UserMusicSettingsStore"/> class rooted in a supplied data folder.
    /// </summary>
    ...
    public UserMusicViewSettings Get(Guid userId)
    {
        ...
        lock (GetUserLock(userId))
        {
            ...
            {
                _logger.LogWarning(exception, "Failed to read MusicSuite settings for user {UserId}", userId);
                return CreateDefault(userId);
            }
            catch (UnauthorizedAccessException exception)
            {
                _logger.LogWarning(exception, "Access denied reading MusicSuite settings for user {UserId}", userId);
                return CreateDefault(userId);
            }
        }
    }
    ...
    {
        var normalized = new UserMusicViewSettings
        {
            SchemaVersion = CurrentSchemaVersion,
            UserId = userId,
        ...
        foreach (var item in NormalizeStringDictionary(settings.SortPreferences, PreferenceKeys))
        {
            normalized.SortPreferences[item.Key] = item.Value;
        }

        foreach (var item in NormalizeBooleanDictionary(settings.EnabledSections, SectionKeys))
        {
            normalized.EnabledSections[item.Key] = item.Value;
        }
    ...
    {
        var clone = new UserMusicViewSettings
        {
        ...
        foreach (var item in settings.SortPreferences)
        {
            clone.SortPreferences[item.Key] = item.Value;
        }

        foreach (var item in settings.EnabledSections)
        {
            clone.EnabledSections[item.Key] = item.Value;
        }
    ...
    {
        _logger.LogWarning(exception, "Corrupt MusicSuite settings file detected: {Path}", path);
        try
        {
