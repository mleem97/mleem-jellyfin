    ...
    function render(data) {
        ...
        content.innerHTML = drives.map(function (drive) {
            ...
            return '<div style="margin-top:10px">'
                + '<div style="display:flex;justify-content:space-between;font-size:12px;margin-bottom:5px"><strong>' + (drive.label || drive.name) + '</strong><span>' + usedPct + '% · ' + fmtBytes(drive.freeBytes || 0) + ' frei</span></div>'
                + '<div style="height:8px;background:#272727;border-radius:999px;overflow:hidden"><div style="height:100%;width:' + usedPct + '%;background:#5591c7"></div></div>'
                + '<div style="font-size:10px;color:#666;margin-top:3px;font-family:monospace">' + drive.name + '</div>'
                + '</div>';
        }).join('') || '<div style="font-size:12px;color:#888">No matching Jellyfin library mounts detected.</div>';
    }

    function load() {
        apiGet('Plugins/HddDisplay/Storage')
            .then(render)
            .catch(function (error) {
                const state = document.querySelector('#hdd-display-widget-state');
                if (state) state.textContent = 'Error';
                const content = document.querySelector('#hdd-display-widget-content');
                if (content) content.innerHTML = '<div style="color:#dd6974;font-size:12px">' + error.message + '</div>';
            });
    }
